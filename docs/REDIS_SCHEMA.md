...
| 3.7 | `stock:rank:up` / `down` / `amount` / `quant` | ZSET | Spark 离线任务 | ❌ 待开发 |
...
---
### 3.7 排行榜 — `stock:rank:up` / `down` / `amount` / `quant` ❌
...
| 文件 | 角色 |
|------|------|
| `core/.../streaming/MarketDataWriter.java` | 入口：日清/跨天检查 + Pipeline 写 Redis + flush → MySQL |
| `core/src/main/resources/redis_ohlcv_upsert.lua` | 日线 OHLCV + 市场汇总 Lua 原子更新 |
| `core/src/main/resources/redis_minute_upsert.lua` | 分钟 OHLCV raw 值记录，乱序处理 |
| `core/src/main/java/com/stock/streaming/QuoteStreamingJob.java` | 调用点：`MarketDataWriter.write(parsedRDD)` + shutdown flush |
