...
|------|------|
| Key 前缀 | `stock:` |
| 不存 name | 股票名称从 MySQL `dim_stock` 查，Redis 只存 code |
...
| # | Redis Key | 类型 | 写入方 | 状态 |
|---|-----------|------|--------|------|
| 3.1 | `stock:market:summary` | Hash | Spark Streaming → `RedisWriter` | ✅ 已实现 |
| 3.2 | `stock:quote:ohlcv:{code}` | String (JSON) | Spark Streaming → `RedisWriter` | ✅ 已实现 |
| 3.3 | `stock:quote:{code}` | String (JSON) | data_gen | ✅ 已由 data_gen 写入 |
| 3.4 | `stock:rank:up` / `down` / `amount` / `quant` | ZSET | Spark 离线任务 | ❌ 待开发 |
| 3.5 | `stock:alert:latest` | List (JSON) | Spark Streaming | ❌ 待开发 |
...
### 3.4 排行榜 — `stock:rank:up` / `down` / `amount` / `quant` ❌

**统一结构**

| 属性 | 值 |
|------|-----|
| 类型 | ZSET（member=code, score=排名依据值） |
| 写入方 | Spark 离线任务（每 5 分钟全量刷新） |
| 读取方 | `RankItemDTO` |
| TTL | 无（`DEL` + `ZADD` 全量替换） |

| Key | score 字段 | 排序方向 | 读命令 |
|-----|-----------|---------|--------|
| `stock:rank:up` | `change_pct`（正值） | 高→低 | `ZREVRANGE stock:rank:up 0 19 WITHSCORES` |
| `stock:rank:down` | `change_pct`（负值） | 低→高 | `ZRANGE stock:rank:down 0 19 WITHSCORES` |
| `stock:rank:amount` | `amount`（成交额，万元） | 高→低 | `ZREVRANGE stock:rank:amount 0 19 WITHSCORES` |
| `stock:rank:quant` | `quant_score`（0-100） | 高→低 | `ZREVRANGE stock:rank:quant 0 19 WITHSCORES` |
...
| 文件 | 角色 |
|------|------|
| `core/src/main/java/com/stock/streaming/RedisWriter.java` | Java 入口：日清检查 + 跨天检查 + `foreachPartition` 并行写 |
| `core/src/main/resources/redis_ohlcv_upsert.lua` | Lua 原子脚本：GET 旧值 → 比较 → HINCRBY(FLOAT) → SET |
| `core/src/main/java/com/stock/streaming/QuoteStreamingJob.java` | 调用点：`RedisWriter.updateMarketData(parsedRDD)` |
...
- 触发：Driver 侧每 batch 前 `HGET stat_date` ≠ 今日  → `DEL stock:market:summary` → `HSET stat_date`
- 跨天 batch（`distinct trade_date > 1`）：`DEL` + 跳过，保证不混日期
- `stock:quote:ohlcv:*` 旧 key 日清后首次 SET 自动覆盖，无需手动清理
...
| 个股 OHLCV | ✅ `stock:quote:ohlcv:{code}` (JSON) | `dws_stock_day` / `dws_stock_minute` |
